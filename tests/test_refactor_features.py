...
def test_refactor_features():
    ...
    # 桌面快捷方式：Linux CI 不具备 PowerShell，验证平台分支的调用契约即可。
    ...
    with mock.patch.object(Path, "home", return_value=fh), mock.patch("sys.platform", "win32"), mock.patch("main._create_windows_shortcut") as create_shortcut:
        _ensure_desktop_shortcut(cfg2)
        create_shortcut.assert_called_once_with(fh / "Desktop")
